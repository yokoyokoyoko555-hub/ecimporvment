...
const querySchema = z.object({
  batch: z.string().uuid(),
  type: z.enum(["ochanoko", "smaregi", "images"]),
});
  ...
  try {
    ...
    if (parsed.data.type === "smaregi") {
      const invalid = products.filter(
        (product) => !product.department_id || !product.product_code,
      );
      if (invalid.length)
        return NextResponse.json(
          {
            error: `スマレジ必須項目が未入力の商品が${invalid.length}件あります。部門ID・商品コードを確認してください。`,
          },
          { status: 422 },
        );
      const lines = [
    ...
    if (parsed.data.type === "ochanoko") {
      ...
      if (headers.length !== 204)
        return NextResponse.json(
          { error: `おちゃのこCSVの列数エラーです（${headers.length}列）` },
          { status: 500 },
        );
      const rows = products.map((product) => {
        ...
        set("商品名", product.product_name);
        set("型番/品番", product.product_code);
        set("カテゴリ", product.category);
    ...
    for (let start = 0; start < products.length; start += 8) {
      await Promise.all(
        products.slice(start, start + 8).map(async (product) => {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "出力処理に失敗しました",
      },
      { status: 500 },
    );
  }
