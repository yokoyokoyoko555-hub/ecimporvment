...
const schema = z
  ...
  .superRefine((value, context) => {
    ...
    if (host !== "takaratomy.co.jp")
      context.addIssue({
        code: "custom",
        path: ["sourceUrl"],
        message: "タカラトミーのロルカナ公式URLを入力してください",
      });
  });
...
export async function POST(request: Request) {
  ...
  const existing = await pool.query<{ id: string }>(
    "SELECT id FROM import_batches WHERE source_type='lorcana' AND lower(trim(set_code))=lower(trim($1)) AND status<>'failed' ORDER BY created_at LIMIT 1",
    [setCode],
  );
  if (existing.rows[0])
    return NextResponse.json(
  ...
  try {
    const cards = await fetchLorcanaCards(sourceUrl, setName, setCode);
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      const batch = await client.query<{ id: string }>(
        `INSERT INTO import_batches(source_type,source_url,set_name,set_code,status)
        VALUES('lorcana',$1,$2,$3,'fetching') RETURNING id`,
      ...
      const templateResult = await client.query<{
        ...
        "SELECT template_text,multiple_colors_label FROM product_name_templates WHERE title_key='lorcana'",
      );
      if (!templateResult.rows[0])
        throw new Error("ロルカナの商品名テンプレートが見つかりません");
      const template = {
      ...
      for (const card of cards) {
        ...
        const productCode = `LC_${card.cardNumber}_N`
          .toUpperCase()
          .replace(/[^A-Z0-9_-]/g, "");
