...
const inputSchema = z.object({
  ...
  if (new URL(value.sourceUrl).hostname !== "digimoncard.com") ctx.addIssue({ code: "custom", path: ["sourceUrl"], message: "デジモンカード公式URLを入力してください" });
});
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const card of cards) {
      const inserted = await client.query<{ id: string }>(`INSERT INTO cards(
