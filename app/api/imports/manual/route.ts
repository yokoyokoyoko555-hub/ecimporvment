...
const cardSchema = z.object({
  ...
  imageUrl: z
    .union([z.string().url("画像URLを確認してください"), z.literal("")])
    .nullable()
    .optional(),
  productCode: z
    .string()
    ...
    .regex(
      /^[A-Za-z0-9_-]*$/,
      "商品コードは半角英数字・ハイフン・アンダーバーで入力してください",
    )
    .optional(),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!template)
      throw new Error("選択したタイトルの商品名テンプレートが見つかりません");

    const totalProducts = parsed.data.cards.reduce(
      (count, card) => count + (card.createDamaged ? 2 : 1),
      0,
    );
    ...
    for (let index = 0; index < parsed.data.cards.length; index += 1) {
      ...
      const cardResult = await client.query<{ id: string }>(
        ...
        [
          ...
          card.colors,
          card.imageUrl || null,
          JSON.stringify({ entryType: "manual" }),
        ],
      );
      const generatedName = renderProductName(
        {
          templateText: template.template_text,
          multipleColorsLabel: template.multiple_colors_label,
        },
        {
          name: card.cardName,
          rarity: card.rarity,
      ...
      await client.query(
        ...
        [
          ...
          productCode,
          card.productName || generatedName,
          card.salePrice ?? null,
          card.costPrice ?? null,
          card.initialStock,
      ...
      if (card.createDamaged) {
        ...
        const existingDamaged = await client.query(
          "SELECT 1 FROM legacy_product_codes WHERE product_code=$1 UNION ALL SELECT 1 FROM products WHERE product_code=$1 LIMIT 1",
          [damagedCode],
        );
        if (existingDamaged.rowCount)
          throw new Error(
            `傷あり商品コード「${damagedCode}」はすでに使用されています`,
          );
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "手動登録に失敗しました",
      },
      { status: 500 },
    );
  } finally {
