...
type CodeRule = {
  ...
  branch_width: number;
  damage_suffix: string | null;
  special_policy: string | null;
...
export default async function SettingsPage() {
  ...
  if (pool) {
    const result = await pool.query<{
      ...
      "SELECT title_key,display_name,template_text,multiple_colors_label FROM product_name_templates ORDER BY created_at",
    );
    if (result.rows.length)
      items = result.rows.map((row) => ({
        ...
        multipleColorsLabel: row.multiple_colors_label,
      }));
    const ruleResult =
      await pool.query<CodeRule>(`SELECT r.display_name,r.base_expression,r.branch_width,r.damage_suffix,r.special_policy,r.source_note,r.analyzed_rows,
    ...
    sources = sourceResult.rows.map((row) => ({
      titleKey: row.title_key,
      displayName: row.display_name,
      sourceName: row.source_name || "手動登録",
      acquisitionMethod: row.acquisition_method || "manual",
      ...
      <header className="topline">
        <div>
          ...
          <p className="subtitle">
            タイトルごとの商品名や出力規則を管理します。
          </p>
        </div>
      </header>
      {sources.length > 0 && <CatalogSourceSettings initialItems={sources} />}
      {codeRule && (
        <section className="panel codeRulePanel">
          <div className="panelHeader">
            <div>
              <h2>商品コード採番ルール</h2>
              <p className="subtitle">
                既存商品と重複しないよう、過去の採番状況を保存しています。
              </p>
            </div>
            <span className="ruleStatus">適用中</span>
          </div>
          <details className="codeRuleDetail">
            <summary>
              <div>
                <b>{codeRule.display_name}</b>
                <span>通常カード：型番のハイフンを除去 ＋ 2桁枝番</span>
              </div>
              <strong>例：OP01-001 → OP01001-01</strong>
            </summary>
            <div className="ruleFacts">
              <div>
                <span>通常カード</span>
                <b>
                  {codeRule.base_expression} + {codeRule.branch_width}桁枝番
                </b>
              </div>
              <div>
                <span>傷あり商品</span>
                <b>元の商品コード + {codeRule.damage_suffix}</b>
              </div>
              <div>
                <span>既存コード予約</span>
                <b>{Number(codeRule.reserved_count).toLocaleString()}件</b>
              </div>
              <div>
                <span>型番別の枝番記録</span>
                <b>{Number(codeRule.sequence_count).toLocaleString()}型番</b>
              </div>
            </div>
            <p>{codeRule.special_policy}</p>
            <small>{codeRule.source_note}</small>
          </details>
          <ProductCodeImport />
        </section>
      )}
