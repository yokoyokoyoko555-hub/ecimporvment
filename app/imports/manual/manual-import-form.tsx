...
type CardInput = { key: string; cardName: string; cardNumber: string; rarity: string; colorsText: string; isParallel: boolean; createDamaged: boolean; imageUrl: string; autoProductCode: boolean; productCode: string; productName: string; salePrice: string; costPrice: string; initialStock: string; departmentId: string; category: string };

const emptyCard = (): CardInput => ({ key: crypto.randomUUID(), cardName: "", cardNumber: "", rarity: "", colorsText: "", isParallel: false, createDamaged: false, imageUrl: "", autoProductCode: true, productCode: "", productName: "", salePrice: "", costPrice: "", initialStock: "0", departmentId: "", category: "" });
...
export function ManualImportForm({ templates }: { templates: ManualTemplate[] }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const payload = { titleKey, setName, setCode, cards: cards.map((card) => ({
      ...
      colors: colorsOf(card.colorsText), isParallel: card.isParallel, createDamaged: card.createDamaged, imageUrl: card.imageUrl || null,
  ...
  return <form className="manualForm" onSubmit={submit}>
    ...
    <div className="manualCards">{cards.map((card, index) => <details className="manualCard" key={card.key} open>
      ...
      <div className="manualCardBody">
        ...
        <div className="manualGrid3"><div className="field"><label>色</label><input value={card.colorsText} onChange={(e) => update(card.key, "colorsText", e.target.value)} placeholder="赤、青" /></div><div className="field"><label>画像URL</label><input type="url" value={card.imageUrl} onChange={(e) => update(card.key, "imageUrl", e.target.value)} placeholder="https://..." /></div><div className="manualChecks"><label className="checkField"><input type="checkbox" checked={card.isParallel} onChange={(e) => update(card.key, "isParallel", e.target.checked)} /> パラレルカード</label><label className="checkField damagedCheck"><input type="checkbox" checked={card.createDamaged} onChange={(e) => update(card.key, "createDamaged", e.target.checked)} /> 傷あり商品も作成</label></div></div>
