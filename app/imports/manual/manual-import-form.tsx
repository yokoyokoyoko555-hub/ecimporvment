...
type CardInput = { key: string; cardName: string; cardNumber: string; rarity: string; colorsText: string; isParallel: boolean; imageUrl: string; productCode: string; productName: string; salePrice: string; costPrice: string; initialStock: string; departmentId: string; category: string };

const emptyCard = (): CardInput => ({ key: crypto.randomUUID(), cardName: "", cardNumber: "", rarity: "", colorsText: "", isParallel: false, imageUrl: "", productCode: "", productName: "", salePrice: "", costPrice: "", initialStock: "0", departmentId: "", category: "" });
...
export function ManualImportForm({ templates }: { templates: ManualTemplate[] }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const payload = { titleKey, setName, setCode, cards: cards.map((card) => ({
      ...
      productCode: card.productCode, productName: card.productName || undefined,
  ...
  return <form className="manualForm" onSubmit={submit}>
    ...
    <div className="manualCards">{cards.map((card, index) => <details className="manualCard" key={card.key} open>
      ...
      <div className="manualCardBody">
        ...
        <div className="manualGrid2"><div className="field"><label>商品コード *</label><input value={card.productCode} onChange={(e) => update(card.key, "productCode", e.target.value)} required pattern="[A-Za-z0-9_-]+" /></div><div className="field"><label>商品名（空欄ならテンプレートで自動作成）</label><input value={card.productName} onChange={(e) => update(card.key, "productName", e.target.value)} placeholder={preview(card)} /></div></div>
